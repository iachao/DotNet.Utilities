...
namespace DotNet.Utilities {
    public static class ExcelHelperForCs
    {
        ...
        private static string GetOpenFilePath()
        {
            ...
            openFileDig.Filter = "Excel Office2007及以上(*.xlsx)|*.xlsx|Excel Office97-2003(*.xls)|*.xls|";
        ...
        public static string ExportToExcel(DataSet sourceDs, string filePath = null)
        {
            ...
            for (int i = 0; i < sourceDs.Tables.Count; i++)
            {
                ...
                {
                    ICell cell = headerRow.CreateCell(column.Ordinal, GetMyCellType(column));
                ...
                foreach (DataRow row in table.Rows)
                {
                    ...
                    foreach (DataColumn column in table.Columns)
                    {
                        if (GetMyCellType(column).Equals(CellType.Numeric))
                        {
                            dataRow.CreateCell(column.Ordinal, GetMyCellType(column)).SetCellValue(double.Parse(row[column].ToString()));
                        }
                        else
                        {
                            if (column.DataType.FullName.Equals(typeof(DateTime).FullName))
                            {
                                ICell myCell = dataRow.CreateCell(column.Ordinal, GetMyCellType(column));

                                DateTimeFormatInfo dtFormat = new DateTimeFormatInfo();
                                myCell.CellStyle = cellStyle;
                                dtFormat.ShortDatePattern = "yyyy/M/d";
                                DateTime tmpDt = Convert.ToDateTime(row[column], dtFormat);
                                myCell.SetCellValue(tmpDt);
                            }
                            else
                            {
                                dataRow.CreateCell(column.Ordinal, GetMyCellType(column)).SetCellValue((row[column]).ToString());
                            }
        ...
        private static CellType GetMyCellType(DataColumn dc)
        {
            if (dc.DataType.FullName.Equals(typeof(string).FullName))
            {
                return CellType.String;
            }
            if (dc.DataType.FullName.Equals(typeof(long).FullName))
            {
                return CellType.Numeric;
            }
            if (dc.DataType.FullName.Equals(typeof(DateTime).FullName))
            {
                return CellType.String;
            }
            if (dc.DataType.FullName.Equals(typeof(decimal).FullName))
            {
                return CellType.Numeric;
            }
            return CellType.String;
